...
const useInfiniteDiscordAccounts = (filters = {}) => {
  ...
  const loadAccounts = async (pageNum = 1, reset = false) => {
    ...
    try {
      ...
      if (result.items && Array.isArray(result.items)) {
        ...
        console.error('Unexpected Discord API response format:', result)
        throw new Error('Invalid response format')
      }
    } catch (err) {
      if (err.name !== 'AbortError') {
        console.error('Error loading Discord accounts:', err)
...
export default useInfiniteDiscordAccounts
