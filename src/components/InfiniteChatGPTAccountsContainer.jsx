import { useInfiniteChatGPTAccounts } from '../hooks/useInfiniteChatGPTAccounts'
import ChatGPTAccountCard from './ChatGPTAccountCard'

// Skeleton component for loading state
const ChatGPTAccountSkeleton = () => (
...
const InfiniteChatGPTAccountsContainer = ({ filters = {} }) => {
  const { accounts, loading, error, hasNextPage, totalItems, loadMore, refresh } =
    useInfiniteChatGPTAccounts(filters)

  console.log('🔍 ChatGPT Container Debug:', {
    ...
    return (
      <div className='flex flex-col items-center justify-center py-20 space-y-4'>
        ...
        <p className='text-gray-400 text-center max-w-md'>
          We couldn't load the ChatGPT accounts. Please check your connection and try again.
        </p>
  ...
  if (accounts.length === 0 && !loading && !error) {
    return (
      <div className='flex flex-col items-center justify-center py-20 space-y-4'>
        <div className='text-6xl'>🤖</div>
        <h3 className='text-xl font-semibold text-gray-200'>No ChatGPT accounts found</h3>
        <p className='text-gray-400 text-center max-w-md'>
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-6'>
        {accounts.map((account, index) => (
          <ChatGPTAccountCard key={`${account.item_id || account.id}-${index}`} account={account} />
        ))}
      </div>
      ...
      {loading && accounts.length > 0 && (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-6'>
          {[...Array(10)].map((_, i) => (
            <ChatGPTAccountSkeleton key={`skeleton-${i}`} />
          ))}
        </div>
      )}
        ...
        {hasNextPage ? (
          ...
          >
            {loading ? (
              ...
              <span>Load More ChatGPT Accounts</span>
            )}
          </button>
        ) : accounts.length > 0 ? (
          <div className='text-center text-gray-400'>
            <p className='text-sm'>You've seen all available ChatGPT accounts</p>
...
export default InfiniteChatGPTAccountsContainer
