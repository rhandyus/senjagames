import { useInfiniteMinecraftAccounts } from '../hooks/useInfiniteMinecraftAccounts'
import MinecraftAccountCard from './MinecraftAccountCard'

const InfiniteMinecraftAccountsContainer = ({ filters = {} }) => {
  const { accounts, loading, loadingMore, error, hasMore, loadMore } =
    useInfiniteMinecraftAccounts(filters)
  ...
  const handleScroll = e => {
    ...
    if (isNearBottom && hasMore && !loading && !loadingMore) {
      console.log('📜 Loading more Minecraft accounts...')
      loadMore()
    }
  }
  ...
  if (error) {
    return (
      <div className='bg-red-900 border border-red-700 text-red-100 px-4 py-3 rounded-lg mb-6'>
        <p className='font-medium'>Error loading Minecraft accounts:</p>
  ...
  if (!accounts || accounts.length === 0) {
    return (
      <div className='text-center py-12'>
        <p className='text-gray-400 text-lg'>No Minecraft accounts found</p>
  ...
  return (
    <div className='space-y-6' onScroll={handleScroll}>
      ...
      <div className='text-sm text-gray-400'>
        <p>{accounts.length} Minecraft accounts loaded</p>
      </div>
      ...
      <div className='account-container grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6'>
        {accounts.map(account => (
          <MinecraftAccountCard
            key={`minecraft-${account.id}-${account.item_id}`}
            account={account}
          />
        ))}
      </div>
      ...
      {!hasMore && accounts.length > 0 && (
        <div className='text-center py-8'>
          <p className='text-gray-500'>You've reached the end of Minecraft accounts</p>
        </div>
      )}
    </div>
  )
...
export default InfiniteMinecraftAccountsContainer
