...
const ChatGPTAccountCard = ({ account }) => {
  ...
  // Get subscription type from title or description
  const getSubscriptionType = () => {
    const title = account.title?.toLowerCase() || ''
    if (title.includes('plus') || title.includes('premium')) return 'ChatGPT Plus'
    if (title.includes('team')) return 'ChatGPT Team'
    if (title.includes('enterprise')) return 'ChatGPT Enterprise'
    if (title.includes('pro')) return 'ChatGPT Pro'
  ...
  const getAccountFeatures = () => {
    ...
    if (title.includes('plus') || desc.includes('plus')) features.push('ChatGPT Plus')
    if (title.includes('api') || desc.includes('api')) features.push('API Access')
    if (title.includes('unlimited') || desc.includes('unlimited')) features.push('Unlimited Usage')
    if (title.includes('gpt-4') || desc.includes('gpt-4')) features.push('GPT-4 Access')
    if (title.includes('plugin') || desc.includes('plugin')) features.push('Plugins')
    if (title.includes('priority') || desc.includes('priority')) features.push('Priority Access')
    ...
    >
      ...
      <div className='bg-gradient-to-r from-gray-800 to-gray-900 px-4 py-3 flex justify-between items-center border-b border-gray-700'>
        <div className='flex items-center space-x-2'>
          ...
          <span className='text-sm text-gray-300 font-medium'>ChatGPT Account</span>
        </div>
      ...
      <div className='p-4 space-y-3 flex-1'>
        ...
        <div className='flex items-center space-x-2 p-2 rounded-lg border bg-gray-800 border-gray-600'>
          <Icon icon='simple-icons:openai' className='text-green-400 text-sm' />
          <span className='text-green-400 text-xs font-medium'>{getSubscriptionType()}</span>
        </div>
      ...
      <div className='bg-gray-800 px-4 py-3 border-t border-gray-700 flex items-center justify-between text-xs mt-auto'>
        <div className='flex items-center space-x-2'>
          <Icon icon='simple-icons:openai' className='text-green-400' />
...
export default ChatGPTAccountCard
