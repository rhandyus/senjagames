...
// Use /api/lzt/chatgpt as alternative via dynamic routing
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const baseURL = 'https://prod-api.lzt.market/chatgpt'
    ...
    return res.status(500).json({
      error: 'Failed to fetch ChatGPT accounts',
...
export default function handler(req, res) {
  res.status(503).json({
    error: 'ChatGPT API temporarily disabled',
    message:
      'This endpoint is commented out due to Vercel 12 function limit. Use /api/lzt/chatgpt instead.'
  })
}
