...
// Use /api/lzt/tiktok as alternative via dynamic routing
...
export default async function handler(req, res) {
  ...
  try {
    console.log('📱 TikTok API called with query:', req.query)
    ...
    const baseURL = 'https://prod-api.lzt.market/tiktok'
    ...
    console.log('✅ TikTok API Success:', data.items?.length || 0, 'items returned')
...
export default function handler(req, res) {
  res.status(503).json({
    error: 'TikTok API temporarily disabled',
    message:
      'This endpoint is commented out due to Vercel 12 function limit. Use /api/lzt/tiktok instead.'
  })
}
