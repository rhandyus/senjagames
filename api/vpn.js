...
// Use /api/lzt/vpn as alternative via dynamic routing

// Vercel Serverless Function for VPN accounts
export default async function handler(req, res) {
  ...
  try {
    ...
    // Build LZT Market API URL for VPN category
    const baseURL = 'https://prod-api.lzt.market'
    const apiURL = new URL('vpn', baseURL)
    ...
    console.log('🔒 VPN API Request:', apiURL.toString())
    ...
    console.log(`✅ VPN API Success: ${data.items?.length || 0} accounts returned`)
...
export default function handler(req, res) {
  res.status(503).json({
    error: 'VPN API temporarily disabled',
    message:
      'This endpoint is commented out due to Vercel 12 function limit. Use /api/lzt/vpn instead.'
  })
}
