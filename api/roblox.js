// Vercel Serverless Function for Roblox accounts
export default async function handler(req, res) {
  ...
  try {
    ...
    // Build LZT Market API URL for Roblox category
    const baseURL = 'https://prod-api.lzt.market'
    const apiURL = new URL('roblox', baseURL)
    ...
    if (min_robux) apiURL.searchParams.append('min_robux', min_robux)
    if (max_robux) apiURL.searchParams.append('max_robux', max_robux)
    if (premium) apiURL.searchParams.append('premium', premium)
    if (has_pets) apiURL.searchParams.append('has_pets', has_pets)
    if (has_limiteds) apiURL.searchParams.append('has_limiteds', has_limiteds)
    if (email_type) apiURL.searchParams.append('email_type', email_type)
